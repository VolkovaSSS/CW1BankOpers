...
def get_user_settings(file_name: str) -> dict:
    ...
    with open(file_name, "r") as file:
        try:
            ...
            message = f"Ошибка формата файла настроек пользователя: {file_name}"
            logger.error(message)
            print(message)
            return {}
...
def get_top(df: pd.DataFrame, num: int = 5) -> list[dict]:
    ...
    top_5 = filtered_df.nlargest(num, "Сумма операции с округлением")
    top_5_list = []
